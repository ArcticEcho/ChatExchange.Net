...
namespace ChatExchangeDotNet
{
    ...
    {
        private static Dictionary<string, HashSet<Cookie>> allCookies = new Dictionary<string, HashSet<Cookie>>();



        public static bool HasCookieKey(string key)
        {
            return key != null && allCookies.ContainsKey(key);
        }
        ...
        public static void AddCookieKey(string key)
        {
            if (!HasCookieKey(key))
            {
                allCookies[key] = new HashSet<Cookie>();
            }
        }
        ...
        public static void RemoveCookies(string key, string name)
        {
            if (HasCookieKey(key))
            {
                allCookies[key] = new HashSet<Cookie>(allCookies[key].Where(x => x.Name != name));
            }
        }
        ...
        public static HttpRes SendRequest(HttpReq reqInfo)
        {
            ...
            {
                lock (allCookies)
                foreach (var cookie in allCookies[reqInfo.CookieKey])
                {
            ...
            {
				lock (allCookies)
                foreach (Cookie c in reqCookies.GetCookies(res.ResponseUri))
                {
                    var existingCookie = allCookies[reqInfo.CookieKey].SingleOrDefault(x => x.Name == c.Name);

                    if (existingCookie != null)
                    {
                        allCookies[reqInfo.CookieKey].Remove(existingCookie);
                    }

                    allCookies[reqInfo.CookieKey].Add(c);
                }
            }
